...
**Another member's vault (admins):** put the member first — `f @ari health` or `f ari health` (the `@` is optional, same as `t @ari`). Without a member the file lands in *your* vault. `@` is reserved for members: an `@name` nobody matches is never silently yours — it is filed for **everyone?** (the household vault; your own vault if you're not an admin), the ack says so and lists the members, and `/undo #hash` moves it back.
